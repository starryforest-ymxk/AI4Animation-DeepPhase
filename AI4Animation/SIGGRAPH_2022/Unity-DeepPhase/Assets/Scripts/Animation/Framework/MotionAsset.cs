...
namespace AI4Animation {
	public class MotionAsset : ScriptableObject {
		...
		public void ClearSequences() {
			Sequences = Array.Empty<Interval>();
		}
		...
		private Module CreateModule(string type, string tag) {
			...
			if(module == null) {
				Debug.LogError("Module of type " + type + " could not be loaded in " + name + ".");
			} else {
		...
		public T AddModule<T>(string tag=null) where T : Module {
			if(HasModule<T>(tag)) {
				Debug.LogError("Module of type " + typeof(T).Name + (tag == null ? string.Empty : " with tag " + tag) + " already exists in " + name + ".");
				return null;
			} else {
